...
def code2net_tree(individual_code, nb_feats, listtree):
    individual_code, nb_view = utils_tree.viewfusion(listtree)
    input_x = []
    x = []
    x_bn = []
    x_dp = []
    outputs = []  # 用于存储每个视图的分类器输出
    ...
    fusion_x = tf.keras.layers.Lambda(l2_norm, name='fusion_features')(fusion_x)

    fusion_logits = tf.keras.layers.Dense(units=classes, activation=None, name='fusion_fc_2')(fusion_x)
    fusion_out = tf.keras.layers.Activation('softmax', name='fusion_output')(fusion_logits)

    model = tf.keras.models.Model(inputs=input_x, outputs=[fusion_out])
    return model

if __name__ == '__main__':
    model = code2net_tree(individual_code=[], nb_feats=[0,1,2,3,4],
                          listtree =['0a', '1a', '2a', '3a', '4a',
                   '-0', '-0', '-0', '-0',],

)
    tf.keras.utils.plot_model(model, to_file='fusion_model.svg', show_shapes=True)
