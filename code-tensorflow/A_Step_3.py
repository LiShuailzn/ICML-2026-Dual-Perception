...
def train_individual(individual_code, result_save_dir='.', gpu='0', seed=SEED):
    ...
    harmful_indices = harmful_indices.astype(int)
    ...
    print(f">>> Clean train samples: {len(clean_indices)} "
          f"(removed {num_total - len(clean_indices)} harmful samples)")
    print(f">>> Pruning ratio: {(num_total - len(clean_indices)) / num_total:.2%}")
    clean_view_train_xx = [x[clean_indices] for x in view_train_xx]
    clean_train_y = train_y[clean_indices]
    tf.keras.utils.set_random_seed(seed)
    model = code2net_tree(individual_code=[], nb_feats=nb_feats, listtree=individual_code)
    num_views = len(clean_view_train_xx)
    adam = tf.keras.optimizers.Adam()
    model.compile(loss='categorical_crossentropy', optimizer=adam, metrics=['acc'])
    os.makedirs(result_save_dir, exist_ok=True)
    checkpoint_filepath = os.path.join(
        result_save_dir, individual_code_str + f'_seed{seed}' + '.h5'
    )
    ...
    csv_filepath = os.path.join(
        result_save_dir, individual_code_str + f'_seed{seed}' + '.csv'
    )
    csv_logger = tf.keras.callbacks.CSVLogger(csv_filepath)
    print(">>> Start training on CLEAN dataset ...")
    print(f"    clean_train_x shapes: {[x.shape for x in clean_view_train_xx]}")
    print(f"    clean_train_y shape: {clean_train_y.shape}")
    model.fit(
        clean_view_train_xx,
        clean_train_y,
        ...
        epochs=epochs,
        verbose=0,
        validation_data=(view_test_xx, test_y),
    ...
    print(f"Model accuracy on test set (clean-trained): {acc:.4f}")
    return individual_code_str + ',' + str(acc)

if __name__ == "__main__":
    individual_code = ['0a', '1a', '2a', '3a', '4a',
                       '-0', '-0', '-0', '-0', ]
    result_save_dir = '/mnt/disk1/lishuai/code-tensorflow/step-3'
    gpu = '0'
